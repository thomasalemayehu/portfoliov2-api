...
class AuthController {
  ...
  public async registerUser(req: Request, res: Response): Promise<void> {
    ...
    else if (!password) throw new Error("Password is required to register");
    else if (!name) throw new Error("Name is required to register");
  ...
  public async loginUser(req: Request, res: Response): Promise<void> {
    ...
    else if (!password) throw new Error("Password is required to login");

    const possibleUser = await Prisma.getInstance().user.findFirst({
      where: { email: email },
    });

    if (!possibleUser) throw new Error("Invalid email and/or password");

    const passwordMatch = await Bcrypt.compare(password, possibleUser.password);

    if (!passwordMatch) throw new Error("Invalid email and/or password");
  ...
  public async resetPassword(req: Request, res: Response): Promise<void> {
    ...
    if (!email) throw new Error("Email is required to change");
    else if (!password) throw new Error("Password is required to change");
    else if (!newPassword)
      throw new Error("New Password is required to change");
