...
import projectDetailStyles from "../assets/styles/ProjectDetail.module.css";
...
function ProjectDetail() {
  ...
  return (
    <div className={projectDetailStyles.container}>
      {data?.leadImage ? (
        <div
          className={projectDetailStyles.leadImage}
          style={{
            backgroundImage: `url(${assetsPath + data?.leadImage})`,
          }}
        ></div>
      ) : (
        <></>
      )}
      <div className={projectDetailStyles.wrapper}>
        <h1 className={projectDetailStyles.title}>{data?.title}</h1>
        <div className={projectDetailStyles.description}>
          {data?.description}
        </div>
        <div className={projectDetailStyles.flexWrapper}>
          <div className={projectDetailStyles.techStackWrapper}>
          ...
          <div className={projectDetailStyles.linkWrapper}>
            <a href={data?.githubLink} target="__blank">
              <svg
                version="1.0"
            ...
            {/* Link Icon */}
            {data?.liveLink && (
              <a href={data.liveLink} target="__blank">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
        ...
        <div className={projectDetailStyles.selectedImagesWrapper}>
          {data?.imageLinks.map((imageLink) => (
            <img src={`${assetsPath}/${imageLink}`} />
          ))}
        </div>
      </div>
    </div>
